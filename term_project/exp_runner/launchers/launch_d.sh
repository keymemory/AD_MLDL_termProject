...
# 사용: cd term_project && bash exp_runner/launchers/launch_d.sh
...
# results 헤더 (없을 때만)
RES=exp_runner/results/results_scaling_d.tsv
if [ ! -f "$RES" ]; then
  echo -e "ID\tBENCH\tM2\tCLUST\tM1\tMETHOD\tR\tMETRIC\tVALUE\tGEN\tIMG_ACC" > "$RES"
fi

echo "[$(date)] Experiment D ScienceQA 시작"
nohup bash exp_runner/workers/worker_d.sh 0 > /tmp/nohup_d_g0.log 2>&1 &
PID0=$!
nohup bash exp_runner/workers/worker_d.sh 1 > /tmp/nohup_d_g1.log 2>&1 &
PID1=$!
