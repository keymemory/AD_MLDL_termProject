...
# 사용: cd term_project && bash exp_runner/launchers/launch_wd.sh
...
RES=exp_runner/results/results_scaling_wd.tsv
if [ ! -f "$RES" ]; then
  echo -e "ID\tBENCH\tM2\tCLUST\tM1\tMETHOD\tR\tMETRIC\tVALUE\tGEN\tIMG_ACC" > "$RES"
fi

echo "[$(date)] Experiment W-D (ScienceQA weighted_avg) 시작"
echo "총 job 수: $(grep -c '^[^#]' exp_runner/jobs/exp_jobs_scaling_wd.tsv)"

nohup bash exp_runner/workers/worker_wd.sh 0 > /tmp/nohup_wd_g0.log 2>&1 &
PID0=$!
nohup bash exp_runner/workers/worker_wd.sh 1 > /tmp/nohup_wd_g1.log 2>&1 &
PID1=$!
