...
# 사용: cd term_project && bash exp_runner/launch_c.sh
...
# results 헤더 (없을 때만)
RES=exp_runner/results_scaling_c.tsv
if [ ! -f "$RES" ]; then
  echo -e "ID\tBENCH\tM2\tCLUST\tM1\tMETHOD\tR\tMETRIC\tVALUE\tGEN" > "$RES"
fi

echo "[$(date)] Experiment C TextVQA 시작"
nohup bash exp_runner/worker_c.sh 0 > /tmp/nohup_c_g0.log 2>&1 &
PID0=$!
nohup bash exp_runner/worker_c.sh 1 > /tmp/nohup_c_g1.log 2>&1 &
PID1=$!
