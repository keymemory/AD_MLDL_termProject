...
def eval_model(args):
    ...
    # [RESUME] 이미 처리된 question_id는 건너뛰고 이어쓰기 (CUDA 간헐 크래시 대비)
    answered_ids = set()
    if os.path.exists(answers_file):
        with open(answers_file, "r") as _f:
            for _line in _f:
                try:
                    answered_ids.add(json.loads(_line)["question_id"])
                except Exception:
                    pass
    if answered_ids:
        before = len(questions)
        questions = [q for q in questions if q["question_id"] not in answered_ids]
        print(f"[RESUME] 기존 답변 {len(answered_ids)}개 발견, {before}개 중 {len(questions)}개만 처리")
    ...
    # [Phase 1] adaptive selection 집계 → .meta (worker.sh가 읽어 results.tsv에 기록)
    ...
    if _log:
        ...
        # [Phase 1] 이미지별 분포 전체 덤프 (분포 분석 스크립트용)
        with open(answers_file + ".dist.jsonl", "w") as _df:
            for _rec in _log:
                _df.write(json.dumps(_rec) + "\n")
        print(f"[Phase1] adaptive meta: avg_M1={_avg_M1:.2f} avg_r={_avg_r:.4f} n={len(_log)}")
...
if __name__ == "__main__":
    ...
    # [Two-Stage] Stage 2 clustering args
